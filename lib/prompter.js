'use strict'
// Code from https://github.com/ngryman/cz-emoji
...
function loadConfig() {
  const getConfig = (obj) => obj && obj.config && obj.config['cz-commit-emoji']
...
function createQuestions(config) {
  ...
  return [
    ...
    {
      type: config.scopes && config.scopes.length ? 'list' : 'input',
      name: 'scope',
      message: 'Specify a scope:',
      choices: config.scopes && [...config.scopes, { name: '[none]', value: '' }]
    },
    {
      type: 'input',
      name: 'subject',
...
function format(answers) {
  ...
  const [emoji, name] = answers.type.split('-')
  const head = truncate([emoji, ...scope, `${name.trim()}: `].join(' ') + answers.subject.trim(), 100)
